...
//      
    ...
    functionnode nextNode=new functionnode();
     ...
     for (int i = 0; i < elementsOfGroup.size(); i++) {  //Current elements in one group
        ...
        if (VerticesCurrentVNF.isEmpty() ) {
            flagTop=false;
            System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
            System.out.println("Request is rejected because there is no substrate candidate for the currentVNFNode: "  );
            System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
        }
        
         if(!VerticesCurrentVNF.isEmpty() && VerticesCurrentVNF!=null){  //there are a number of candidates for the currentVNFNode mapping
     ...
     // Now we have the getFunc which has the path for all the current nodes from tail. We want to choose the minimum
     int minDistance=100;
     for (int i = 0; i < getFunc.size(); i++) {
         getFunction currentPair=getFunc.get(i);
   System.out.println("Pair that is returned: " + " for the VNF: " + currentPair.getCurrentNode().getNumber() + " the list: " + currentPair.getReturnSPHop());
         ...
         if (currentPair.getReturnSPHop().size()<minDistance){
           ...
           System.out.println("Final pair that has the shortest distance to the tail: " + " for the VNF: " + nextNode.getNumber() + " the list: " + returnedFromSPNewFinal);
 ...
 /////////////////////////////////////////
  ...
  // chains=groups;
   ...
   //Add children of ð�‘‰1 to the child list if its parent is not in the child list
   for (int i = 0; i < PL.size(); i++) {           
        ...
        for (int i = 0; i < groups.size(); i++) {
          ...
          for (int i = 0; i < mappedChain.size(); i++) {
            for (int j = 0; j < mappedChain.get(i).size(); j++) {  
                ...
                System.out.println("mappedChain: " + i + " Current element: " + mappedChain.get(i).get(j).getNumber() );              
            }
            System.out.println("**********************************");           
        } 
    ...
    while (groups.get(k).size() > 0 && flagTop) {      
        if (k==0){ //For the elements of the group 0, the mapping is different:
          ...
          if (usedSub.size()==1){ //Just one VNF is mapped up to now
           ...
           selectedVNF=getNextNodeOtherGroups(SP, treemap, groups.get(0),bw, getNodeVer(usedSub.get(0)),flagTop); //After this function we know which node is mapped next and we have its SP
        ...
        //For each element in group 0 get the  distance from head and tail            
              ...
              selectedVNF=getNextNodeFirstGroup(SP, treemap, groups.get(0),bw, getNodeVer(usedSub.get(0)),getNodeVer(usedSub.get(usedSub.size()-1)),flagTop);
            ...
            temp2=selectedVNF.getReturnSPHop().get(selectedVNF.getReturnSPHop().size()-1);
              if (temp1==usedSub.get(0)){ //SP was found to the head of the current usedSub, so the usedSub should be updated from the start by adding to element 0
                  usedSub.add(0, temp2);                 
              }
              
              if (temp1==usedSub.get(usedSub.size()-1)){ //SP was found to the head of the current usedSub, so the usedSub should be updated from the start by adding to element 0
                  usedSub.add(temp2);                 
              }
              
              // Check if the shortest distance was found to the end of used sub or start of the used sub
        ...
        // usedSub.add(removeVertexNode.getNumber()); 
         ...
         //groupSizeOrg >> original size for group 1, we check this one to differentiate first element of group 1 from rest of them
         ...
         if (selectedVNF_g1.getCurrentNode()==null || selectedVNF_g1.getReturnSPHop().isEmpty()) {
            ...
            temp2g2=selectedVNF_g1.getReturnSPHop().get(selectedVNF_g1.getReturnSPHop().size()-1);
              if (temp1g1==usedSub.get(0)){ //SP was found to the head of the current usedSub, so the usedSub should be updated from the start by adding to element 0
                 // usedSub.add(0, temp2g2);
                  //Direstion of chain is decided here
                  usedSub.add(temp2g2); 
              }
              
              if (temp1g1==usedSub.get(usedSub.size()-1)){ //SP was found to the head of the current usedSub, so the usedSub should be updated from the start by adding to element 0
                  usedSub.add(temp2g2);                 
              }
        ...
        System.out.println("Current UsedSub in g1 after mapping the first element: " + usedSub);
          ...
          selectedVNF_rest=getNextNodeOtherGroups(SP, treemap, groups.get(k),bw, getNodeVer(usedSub.get(usedSub.size()-1)),flagTop); 
          ...
          // Now remove the element from group
           ...
           System.out.println("Current UsedSub for the rest: " + usedSub);
             ...
             if (usedSub.size()==VNFs.size()){
                 System.out.println("Cost: "+ finalCostDC);
            ...
            for (int j = 0; j < mappedChainFinal.get(i).size(); j++) {  
                ...
                System.out.println("mappedChain: " + i + " Current element: " + mappedChainFinal.get(i).get(j).getNumber() );              
            }
            System.out.println("**********************************");           
        }   
          ...
          // TuneUp TTU=new TuneUp(mappedChainFinal, Vertices, Edges, VNFs, filename, bw,treemapFinal,usedSub,count );
          // TTU.tuneUpAlgorithm();
