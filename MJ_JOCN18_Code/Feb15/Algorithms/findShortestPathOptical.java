...
import Algorithms.OriginalMap;
import Algorithms.ReverseMap;
 ...
 */
public class findShortestPathOptical {
	...
	public findShortestPathOptical(ArrayList<Vertex> Vertices, ArrayList<graphEdge> Edges,
	...
	/////////////////////////////////////////////////////////////////////
	// public TreeMap<Integer, ArrayList<Integer>> findShortest(ArrayList<Vertex>
	///////////////////////////////////////////////////////////////////// listofNodesTocheck,Vertex
	///////////////////////////////////////////////////////////////////// source,int
	///////////////////////////////////////////////////////////////////// initialRate){
	// get the source node and list of the nodes that we need to compare the DP,
	///////////////////////////////////////////////////////////////////// return the
	///////////////////////////////////////////////////////////////////// one with
	///////////////////////////////////////////////////////////////////// min
	public ArrayList<Integer> findShortest(ArrayList<Vertex> listofNodesTocheck, Vertex source, int initialRate) {

		// Here I received a list of candidate nodes that we want to check to see which
		// one is closesst to the latest mapped node
		// For example the substrate list {2,3,5,6,11}, and the latest used substrate
		// node that was used for mapping 0
		// so we find the shortest distance between (0,2),(0,3)(0,5)(0,6)(0,11), which
		// is as folloing:
		// the one we the shorest distance is selcted for mapping of the next function
		int dist = 0;
		int minDistance1 = 99999;
		Vertex finalDestination2 = null;
		DijkstraAlgorithm1 Da = new DijkstraAlgorithm1(Vertices, Edges);
		...
		System.out
				.println("*******************************In the shortest path function*******************************");
		...
		for (Vertex vertex : CanListToCheckDij) {
			...
			System.out.println("Current candidate  " + vertex.getNumber() + " with distance of: " + dist
					+ " from node: " + source.getNumber());
		...
		for (int edgeNo = 0; edgeNo < Edges.size(); edgeNo++) {
			...
			for (int i = 0; i < nodesOnSP.size() - 1; i++) {
				...
				if (source == sourceV && destination == destinationV) {
					System.out.println("The  sourceV:" + sourceV + " destinationV: " + destinationV + " Previous: BW:"
							+ Edges.get(edgeNo).getBW());
					Edges.get(edgeNo).setBW(currentBW - bw);
					System.out.println("The upated sourceV:" + sourceV + " destinationV: " + destinationV);
					System.out.println("The upated sourceV:" + sourceV + " destinationV: " + destinationV
							+ " Updated: BW:" + Edges.get(edgeNo).getBW());
					for (int edgeNo2 = 0; edgeNo2 < Edges.size(); edgeNo2++) {
						...
						if (source2 == destinationV && destination2 == sourceV) {
							...
							total_cost += bw;
							System.out.println(" source2:" + source2 + " destination2: " + destination2 + " BW:"
									+ Edges.get(edgeNo2).getBW());
						}
					}
				}
	...
	public void updateCPU(ArrayList<OriginalMap> originalRelation, functionnode toRemoveVNF, Vertex toRmoveVertex) {
		// the CPU for all the used substrate nodes should be updated based on the
		// relation

		System.out.println("Updating the CPU for the used substrate node");
		for (OriginalMap om : originalRelation) {
			if (om.getKey().getNumber() == toRemoveVNF.getNumber()
					&& om.getValue().getNumber() == toRmoveVertex.getNumber()) {
				System.out.println("original : " + " key (VNF): " + om.getKey().getNumber() + " CPU:"
						+ om.key.getReqCPU() + " value (substrate node): " + om.getValue().getNumber() + " CPU: "
						+ om.getValue().getOfferedCPU());
				System.out.println("******************************************");
				Vertex v = om.getValue();
				System.out.println("******************************************");
				System.out.println(
						"previous cpu of the substrate node: " + v.getOfferedCPU() + " Node number:  " + v.getNumber());
				// for each of the vertex. get their key, which is the functionnode, obtain its
				// requested CPU to update the offered CPU of the substrate node
	...
	public TreeMap<Integer, ArrayList<Integer>> generateCandidateList(ArrayList<functionnode> priorityListNodesbackUP) {
		...
		///////////////////////////////////////////////////////////////////////////////
		TreeMap<Integer, ArrayList<Integer>> listTrackTreeMap2 = new TreeMap<Integer, ArrayList<Integer>>(); // to keep
																												// track
																												// of
																												// the
																												// functionnode
		...
		for (int k = 0; k < priorityListNodesbackUP.size(); k++) {
			...
			System.out.println("*****************************************************");
			System.out.println(" VNF: " + priorityListNodesbackUP.get(k).getNumber() + " Requested CPU: "
					+ priorityListNodesbackUP.get(k).getReqCPU() + " Candidates " + lists[k]);
			...
			// Using listTrackTreeMap to jeep joint track of funnctionode and its list of
			// vertices
		}
	...
	public void removeVNF(TreeMap<Integer, ArrayList<Integer>> listTrackTreeMap2, functionnode toRemoveVNF,
		...
		for (Integer f : listTrackTreeMap2.keySet()) {
			...
			System.out.println("*****************************************************");
			System.out.println("CandidateList at the end " + " the removed substrate node: " + toRmoveVertex.getNumber()
					+ " " + candidateList);
		}
		System.out.println(
				".......................................................................................................");
	...
	public ArrayList<OriginalMap> mappingRelation(functionnode VNF, Vertex substrateNode) {
		...
		for (OriginalMap om : originalRelation) {
			System.out.println(
					"original : " + " key: " + om.getKey().getNumber() + " value: " + om.getValue().getNumber());
		}
		for (ReverseMap rm : reversedrelation) {
			System.out.println(
					"reversed: " + " key: " + rm.getKey().getNumber() + " value: " + rm.getValue().getNumber());
		}
		System.out.println(
				".......................................................................................................");
		return originalRelation;
	...
	public void getVertexRelation(functionnode VNF) { // get a functionnode and return the substrate node that it is
														// mapped to
