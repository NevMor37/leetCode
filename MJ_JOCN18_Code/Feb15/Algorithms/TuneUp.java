...
public class TuneUp {
	...
	// String file, int BW) {
	public TuneUp(ArrayList<ArrayList<Vertex>> mappedChain, ArrayList<Vertex> substrateGraph,
	...
	public void tuneUpAlgorithm() {
		...
		for (int i = 0; i < this.givenChain.size(); i++) {
			for (int j = 0; j < this.givenChain.get(i).size(); j++) {
				...
				System.out.println(
						"mappedChain: " + i + " Current element: " + this.givenChain.get(i).get(j).getNumber());
			}
			System.out.println("**********************************");
		}
	...
	private ArrayList<ArrayList<Vertex>> triangleTuneUp(ArrayList<Vertex> substrateNodes,
			ArrayList<ArrayList<Vertex>> currentChain, ArrayList<Vertex> currentNode, int yValue) {
		...
		ArrayList<Vertex> substrateNeighborCandidates = substrateNeighborCandidates(currentNode, this.usedSub);// this
																												// method
																												// needs
																												// to
																												// take
		...
		for (Vertex currentNeighborCandidate : substrateNeighborCandidates) {
			...
			if (chainWithOutGroup.indexOf(currentNode.get(0)) != chainWithOutGroup.size() - 1) {
				shortestToNext = shortestPath.findShortest(nodes,
						chainWithOutGroup.get(chainWithOutGroup.indexOf(currentNode.get(0)) + 1), this.bandWidth);
			} else {
				shortestToNext = null;
			}
			...
			// ArrayList<Integer> shortestToNext=shortestPath.findShortest(nodes,
			// chainWithOutGroup.get(chainWithOutGroup.indexOf(currentNode.get(0))+1),this.bandWidth);
			...
			if (chainWithOutGroup.indexOf(currentNode.get(0)) != 0) {
				shortestToPre = shortestPath.findShortest(nodes,
						chainWithOutGroup.get(chainWithOutGroup.indexOf(currentNode.get(0)) - 1), this.bandWidth);
			} else {
				shortestToPre = null;
			}
	...
	private ArrayList<Vertex> substrateNeighborCandidates(ArrayList<Vertex> currentNode,
			ArrayList<Integer> usedSubSFC) {
